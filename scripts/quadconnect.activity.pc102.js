...
(() => {
  ...
  function syncUi() {
    if (MOD.elStatus) MOD.elStatus.textContent = MOD.statusText;
    if (MOD.elBtnQuestion) {
      MOD.elBtnQuestion.disabled = !(MOD.currentStudent && MOD.qb && MOD.cat && MOD.sub && MOD.turnState === 'awaitingQuestion');
    }
    if (MOD.elBtnNextTurn) {
      MOD.elBtnNextTurn.disabled = (MOD.turnState !== 'resolved');
    }
  }
  ...
  function openQuestionModal(student) {
    ...
    modal.innerHTML = `
      <div class="qc-modal-card" role="dialog" aria-modal="true" aria-label="Quad Connect Question">
        ...
        <div class="qc-footer">
          <button class="ce-qc-btn" data-qc-reveal disabled>👁 Show Answer</button>
  ...
  function selectNextTurn() {
    ...
    setStatus(`${team.label} turn. Open a question for ${picked.name}.`);
  ...
  function bindUi() {
    ...
    MOD.elBtnQuestion = qs('[data-qc-open-question]', MOD.root);
    MOD.elBtnNextTurn = qs('[data-qc-next-turn]', MOD.root);
    ...
    const btn = qs('[data-qc-next-turn]', MOD.root);
    if (btn) {
    ...
    if (MOD.elBtnQuestion) {
      MOD.elBtnQuestion.addEventListener('click', () => {
        if (MOD.turnState !== 'awaitingQuestion') return;
      ...
      <div class="ce-qc-root">
        <div class="ce-qc-card">
          ...
          <div class="ce-qc-turnbox">
            ...
            <button type="button" class="ce-qc-btn qc-open" data-qc-open-question>Open Question</button>
            <button type="button" class="ce-qc-btn" data-qc-next-turn>Next Turn</button>
          </div>
        </div>
        ${buildBoardHtml()}
      </div>
  ...
  function hardReset() {
    ...
    MOD.elCat = null;
    MOD.elSub = null;
    MOD.elBtnQuestion = null;
    MOD.elBtnNextTurn = null;
  }
